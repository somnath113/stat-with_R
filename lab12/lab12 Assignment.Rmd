...
For the case-1 we have random effect(we are comparing variance, actually we are interested in all car-types), whereas for the case-2 we have fixed effect (we are comparing the means for specific car-type A,B,D).  

### Q4. Which model is more appropriate? Why?
Random effect model is more appropriate. The reason is that we don’t consider the the difference between these specific 4 car-types, rather all car-types.      
